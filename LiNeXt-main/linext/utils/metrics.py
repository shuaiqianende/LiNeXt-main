    ...
    def update(self, gt, pred):
        ...
        for i, vsize in enumerate(self.voxel_sizes):
            ...
            hist_gt = np.histogramdd(
                ...
                # range = ([-max_range, max_range], [-max_range, max_range], [-max_range, max_range])
            ...
            # gt_sum0 = (hist_gt == 0).sum()
            # pre_sum0 = (hist_pred == 0).sum()
            # print('tp fn fp tn numpy', tp, fn, fp, tn, gt_sum, pre_sum, gt_sum0, pre_sum0, vsize)
            # print('pred.points', np.array(pred.points).shape)
            # print('gt.points', np.array(gt.points).shape)
