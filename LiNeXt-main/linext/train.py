...
import linext.datasets.datasets as datasets
import linext.models.models as models
...
def main(config, weights, checkpoint, test):
    ...
    #Load data and model
    if weights is None:
        model = models.DenoisePoints(cfg)
    else:
        if test:
        ...
        model = models.DenoisePoints.load_from_checkpoint(weights, hparams=cfg)
        print(model.hparams)
