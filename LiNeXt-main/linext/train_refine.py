...
import linext.datasets.datasets_refine as datasets
import linext.models.models_refine as models
...
def main(config, weights, checkpoint, test):
    ...
    if weights is None:
        model = models.RefineModule(cfg)
    else:
        model = models.RefineModule.load_from_checkpoint(weights,hparams=cfg)
    ...
    if torch.cuda.device_count() > 1:
        ...
        trainer = Trainer(gpus=cfg['train']['n_gpus'],
                          ...
                          # accelerator='ddp',
                          accelerator='cuda',
                          # strategy=DDPStrategy(start_method='fork'),  # ← DDP + fork
